...
Reads:
  ...
  - Testcase review records from <target>/<domain>/<app>/qa-review-testcase-r{1,2}.json
...
def _collect_qa_records() -> str:
    ...
    sections = []
    for role in ("testcase",):
        records = sorted(app_dir.glob(f"qa-review-{role}-r*.json"))
        if not records:
            continue

        title = "Testcase Creator ↔ Testcase QA"
        ...
        ai_path = app_dir / "test-ai-result.json"
        if ai_path.exists():
            try:
